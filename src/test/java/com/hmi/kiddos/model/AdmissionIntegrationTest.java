...
public class AdmissionIntegrationTest {
	...
	public void testFindAllAdmissions() {
		...
		long count = admissionDao.countAdmissions();
		Assert.assertTrue(
				"Too expensive to perform a find all test for 'Admission', as there are " + count
	...
	public void testFindAdmissionEntries() {
		...
		Assert.assertEquals("Find entries method for 'Admission' returned an incorrect number of entries", count,
				result.size());
	}
	...
	public void testFlush() {
		...
		obj.flush();
		Assert.assertTrue("Version for 'Admission' failed to increment on flush directive",
	...
	public void testMergeUpdate() {
		...
		Assert.assertEquals("Identifier of merged object not the same as identifier of original object", merged.getId(),
				id);
		Assert.assertTrue("Version for 'Admission' failed to increment on merge and flush directive",
				(currentVersion != null && obj.getVersion() > currentVersion) || !modified);
	}
